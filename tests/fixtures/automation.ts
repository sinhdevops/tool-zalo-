...
export function automationFixture(store = new AutomationStore(), settleMs = 0) {
  ...
  const service = new AutomationService(accounts, store, settleMs)
  const emit = (event: IncomingMessage) => { for (const fn of listeners) fn(event) }
  return { service, store, calls, accounts, emit, failForward: () => { failForward = true }, holdHeart: (fn: () => Promise<void>) => { onHeart = fn }, async enable() { await service.configure('99', '22', '11', '33'); await service.toggle(true) }, async drain() { for (let i = 0; i < 10; i++) { await service.tick(); await new Promise<void>((resolve) => setImmediate(resolve)) } } }
}
